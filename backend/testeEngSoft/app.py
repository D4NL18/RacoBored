...
# Configuração do CORS - Permite solicitações do frontend (por padrão localhost:4200 para Angular)
CORS(app, origins=["http://localhost:4200"])  # Substitua com o endereço correto do seu frontend
...
# Inicializa o banco de dados e cria tarefas predefinidas
...
def login():
    ...
    if usuario and bcrypt.check_password_hash(usuario.senha, senha):
        return jsonify({
            ...
            'nome': usuario.nome,
            'email': usuario.email,
            'pontos': usuario.pontos,  # Adiciona os pontos do usuário
        }), 200
    else:
        return jsonify({'message': 'Credenciais inválidas'}), 401

# Rota para retornar duas tarefas aleatórias
...
# Rota para obter o histórico de tarefas do usuário
...
# Rota para completar a tarefa do usuário (status "concluída")
...
def pending_task(user_id):
    # Busca a tarefa pendente do usuário
